...
You can execute the `GetAssessmentsForCourse` Query using the following Query hook function, which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts):

```javascript
useGetAssessmentsForCourse(dc: DataConnect, vars: GetAssessmentsForCourseVariables, options?: useDataConnectQueryOptions<GetAssessmentsForCourseData>): UseDataConnectQueryResult<GetAssessmentsForCourseData, GetAssessmentsForCourseVariables>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useGetAssessmentsForCourse(vars: GetAssessmentsForCourseVariables, options?: useDataConnectQueryOptions<GetAssessmentsForCourseData>): UseDataConnectQueryResult<GetAssessmentsForCourseData, GetAssessmentsForCourseVariables>;
```
...
```
### Return Type
Recall that calling the `GetAssessmentsForCourse` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `GetAssessmentsForCourse` Query is of type `GetAssessmentsForCourseData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `GetAssessmentsForCourse`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, GetAssessmentsForCourseVariables } from '@dataconnect/generated';
import { useGetAssessmentsForCourse } from '@dataconnect/generated/react'

export default function GetAssessmentsForCourseComponent() {
  // The `useGetAssessmentsForCourse` Query hook requires an argument of type `GetAssessmentsForCourseVariables`:
  ...
  // You don't have to do anything to "execute" the Query.
  // Call the Query hook function to get a `UseQueryResult` object which holds the state of your Query.
  const query = useGetAssessmentsForCourse(getAssessmentsForCourseVars);
  // Variables can be defined inline as well.
  const query = useGetAssessmentsForCourse({ courseId: ..., });

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useGetAssessmentsForCourse(dataConnect, getAssessmentsForCourseVars);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useGetAssessmentsForCourse(getAssessmentsForCourseVars, options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useGetAssessmentsForCourse(dataConnect, getAssessmentsForCourseVars, options);
...
```javascript
useUpdateAssessment(options?: useDataConnectMutationOptions<UpdateAssessmentData, FirebaseError, UpdateAssessmentVariables>): UseDataConnectMutationResult<UpdateAssessmentData, UpdateAssessmentVariables>;
```
You can also pass in a `DataConnect` instance to the Mutation hook function.
```javascript
useUpdateAssessment(dc: DataConnect, options?: useDataConnectMutationOptions<UpdateAssessmentData, FirebaseError, UpdateAssessmentVariables>): UseDataConnectMutationResult<UpdateAssessmentData, UpdateAssessmentVariables>;
```
...
```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, UpdateAssessmentVariables } from '@dataconnect/generated';
...
export default function UpdateAssessmentComponent() {
  ...
  // After calling the Mutation hook function, you must call `UseMutationResult.mutate()` to execute the Mutation.
  // The `useUpdateAssessment` Mutation requires an argument of type `UpdateAssessmentVariables`:
  const updateAssessmentVars: UpdateAssessmentVariables = {
    id: ..., 
    score: ..., // optional
  };
  mutation.mutate(updateAssessmentVars);
  // Variables can be defined inline as well.
  ...
  mutation.mutate(updateAssessmentVars, options);
